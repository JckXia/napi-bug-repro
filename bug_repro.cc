...
class MyObjWrap: public Napi::ObjectWrap <MyObjWrap> {
  ...
  public: static Napi::FunctionReference Initialize(Napi::Env env) {
  ...
  private: void Print(const Napi::CallbackInfo & info) {
...
class MyObjWrap1: public Napi::ObjectWrap<MyObjWrap1> {
  ...
  private: void Print1(const Napi::CallbackInfo & info) {
...
void threadLoop() {
  while (!isStop) {
    auto callback = [ = ](Napi::Env env, Napi::Function
      function) {
...
class MyObjWrap2: public Napi::ObjectWrap<MyObjWrap2> {
  ...
  private: void Print2(const Napi::CallbackInfo & info) {
...
class CustomAddon: public Napi::Addon<CustomAddon> {
  ...
  private: Napi::FunctionReference myObjWrapCtr;
...
//NODE_API_NAMED_ADDON(repro, CustomAddon)
